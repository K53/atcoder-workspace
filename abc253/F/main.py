...
def main():
    ...
    # offsets = [0] * N
    ans = [0] * Q
    seg = SegTree(0, [0] * (M + 1), lambda a, b: a + b)
    last_imp = [-1] * N # 行N：最後のクエリQ
    last_implace = {} # クエリQ: 最後の代入クエリ番号Q
    imp_to_ans = defaultdict(list) # クエリQ: 最後の代入クエリ番号Q
    ...
    for qq in range(Q):
        ...
        if t == 1:
            ...
            i -= 1
            j -= 1
            last_implace[qq] = last_imp[i]
            imp_to_ans[last_imp[i]].append(qq)
            queries.append((t, i, j))
    # print(last_implace)
    # print(imp_to_ans)
    ...
    for qq in range(Q):
        ...
        if t == 1:
            ...
            seg.pointAdd(l, x)
            seg.pointAdd(r + 1, -x)
            # print(seg.getRange(0, 1) - seg.getRange(0, 0))
            # print(seg.getRange(0, 2) - seg.getRange(0, 0))
            # print(seg.getRange(0, 3) - seg.getRange(0, 0))
            # print(seg.tree)
        elif t == 2:
            i, x = args
            # offsets[i] = x
            for qq_val in imp_to_ans[qq]:
                j = queries[qq_val][2]
                ans[qq_val] += x - seg.getRange(0, j + 1)
                # print(x, seg.getRange(0, j + 1))
                # print(seg.tree)
        else:
            i, j = args
            print(ans[qq] + seg.getRange(0, j + 1))
            # print("#", ans[qq], seg.getRange(0, j + 1))
    return
