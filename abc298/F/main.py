...
def solve(N: int, r: "List[int]", c: "List[int]", x: "List[int]"):
    compressed_r = {val : index for index, val in enumerate(sorted(list(set(r))))}
    compressed_c = {val : index for index, val in enumerate(sorted(list(set(c))))}
    sum_r = [0] * len(compressed_r)
    sum_c = [0] * len(compressed_c)
    idx_r = [set() for _ in range(len(compressed_r))]

    for rr, cc, xx in zip(r, c, x):
        rr, cc = compressed_r[rr], compressed_c[cc]
        sum_r[rr] += xx
        sum_c[cc] += xx
        idx_r[rr].add((cc, xx))
    ans = 0
    seg = SegTree(0, sum_c, max)
    for rr in range(len(compressed_r)):
        for cc, xx in idx_r[rr]:
            seg.pointAdd(cc, -xx)
        ans = max(ans, sum_r[rr] + seg.getRange(0, len(compressed_c)))
        for cc, xx in idx_r[rr]:
            seg.pointAdd(cc, xx)
    print(ans)
    return
