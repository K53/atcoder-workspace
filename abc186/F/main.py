...
def solve(H: int, W: int, M: int, X: "List[int]", Y: "List[int]"):
    # M += 1
    # X.append(1)
    # Y.append(3)
    column_head_block = defaultdict(lambda :H)
    row_blocks = defaultdict(list)
    row_head_block = defaultdict(lambda :W)
    for i in range(M):
        column_head_block[Y[i] - 1] = min(column_head_block[Y[i] - 1], X[i] - 1)
        row_blocks[X[i] - 1].append(Y[i] - 1)
        row_head_block[X[i] - 1] = min(row_head_block[X[i] - 1], Y[i] - 1)
    # print(column_head_block)
    ans = 0
    for i in range(W):
        head = column_head_block[i]
        ans += head
        if column_head_block[i] == 0:
            row_blocks[0].extend(list(range(i + 1, W)))
            break
    # print(ans)
    # print(row_blocks)
    # print(row_head_block)
    seg = SegTree(0, [0] * W, lambda a, b: a + b)
    for i in range(H):
        if i != 0:
            ans += seg.getRange(0, row_head_block[i])
        if row_head_block[i] == 0:
            break
        for bl in row_blocks[i]:
            if bl != W:
                seg.pointUpdate(bl, 1)
    print(ans)
    return
