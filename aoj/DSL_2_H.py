...
class LazySegTree:
    ...
    @classmethod
    def opeB(cls, org, late, tmp_w): # 遅延配列→値配列への伝播処理(処理を実際にどうするのか)
        if org is None: org = cls.unit_lazy
        return org + late

    @classmethod
    def calc(cls, left, right): # 値配列内での遡上処理(より大きいセグメントへの伝播)
    ...
    def __init__(self, unit: int, bottomList: "list[int]", func: "function", updateFunc: "function", propagateFunc: "function", isLogging: bool = False, convertLengthToThePowerOf2: bool = False):
        ...
        self.value = [self.unit_init] * self.segLen
        self.lazy = [None] * self.segLen
        self.isLogging = isLogging
        if self.isLogging:
            self.logvalue = [len(str(self.unit_init)) + 2] * self.segLen
    ...
    def _propagateAt(self, idx: int):
        if self.lazy[idx] is None:
            return
        # # 値配列側に反映
        tmp_w = 2 ** (self.segDepth - (idx << 1).bit_length())
        self.value[idx << 1] = self.opeB(self.value[idx << 1], self.lazy[idx], tmp_w)
        tmp_w = 2 ** (self.segDepth - (idx << 1 | 1).bit_length())
        self.value[idx << 1 | 1] = self.opeB(self.value[idx << 1 | 1], self.lazy[idx], tmp_w)
    ...
    def rangeUpdate(self, l: int, r: int, x: int):
        ...
        while l < r:
            if l & 1: # 奇数(=右側のセグメント)なら配列を更新、右の部屋に行って(+1)から上層へ(>>1)。
                tmp_w = 2 ** (self.segDepth - l.bit_length())
                self.lazy[l] = self.opeA(self.lazy[l], x)
                self.value[l] = self.opeB(self.value[l], x, tmp_w)
                l += 1
            if r & 1:
                r -= 1
                tmp_w = 2 ** (self.segDepth - r.bit_length())
                self.lazy[r] = self.opeA(self.lazy[r], x)
                self.value[r] = self.opeB(self.value[r], x, tmp_w)
            l >>= 1
            r >>= 1
            cellWidth <<= 1
        ...
        for i in related_indices:
            self.value[i] = self.calc(self.value[i << 1], self.value[i << 1 | 1])
...
def main():
    ...
    seg = LazySegTree(0, [0] * N, min, updateFunc, propageteFunc, True, True)
    for _ in range(Q):
