...
def solve(N: int, h: "List[int]", w: "List[int]", d: "List[int]"):
    ...
    seg = SegTree(INF, [INF] * len(raw_to_compressed), min)

    t = []
    for ll in l:
        hh, ww, dd = ll
        if len(t) > 0 and t[0][0] < hh:
            for tt in t:
                if tt[2] < seg.getPoint(raw_to_compressed[tt[1]]):
                    seg.pointUpdate(raw_to_compressed[tt[1]], tt[2])
            t = []

        if seg.getRange(0, raw_to_compressed[ww]) < dd:
            print(YES)
            return
