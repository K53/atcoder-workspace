...
class LazySegTree:
    ...
    def _recalc_above(self, m: int):
        while m > 1:
            m >>= 1
            self.tree[m] = self.func(self._lazyEval(m << 1), self._lazyEval(m << 1 | 1))
    ...
    def rangeOperate(self, l: int, r: int, val: int):
        """
        区間更新
        変更は遅延配列にマージするのみ。
        O(log(self.bottomLen))
        ...
        self._recalc_above(l)
        self._recalc_above(r)
