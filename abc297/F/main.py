...
def solve(H: int, W: int, K: int):
    ...
    for hh in range(1, H + 1):
        for ww in range(1, W + 1):
            ...
            a += tmp
            # a %= MOD
            # print(comb.nCr(height * width, K - 2) * height * width)
            # print(hh, ww, hh2, ww2)

    # print(a)
    for hh in range(1, H + 1):
        for ww in range(1, W + 1):
            if hh * ww <= 2 or hh * ww - 4 < K - 2:
                continue
            # print(hh, ww, "#", height * width)
            if hh == 1 or ww == 1:
                continue
            tmp = comb.nCr(hh * ww - 4, K - 2)
            # print(hh, ww, tmp)
            tmp *= hh
            # tmp %= MOD
            tmp *= ww
    ...
    # a //= gg
    # mm //= gg
    print(a * pow(mm, MOD - 2, MOD) % MOD)
    # print(comb.nCr(H * W, K))
    return
