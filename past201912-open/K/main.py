...
class LcaDoubling:
    ...
    def isOnPath(self, nodeA: int, nodeB: int, evalNode: int):
        # print(self.getDistance(nodeA, nodeB))
        # print(self.getDistance(nodeA, evalNode))
        # print(self.getDistance(evalNode, nodeB))
...
def solve(N: int, p: "List[int]", Q: int, a: "List[int]", b: "List[int]"):
    president = p.index(-1)
    ld = LcaDoubling(N, root=president)
    for i in range(N):
        if p[i] == -1:
            president = i
            continue
        ld.addEdge(p[i] - 1, i, cost=1)
    ld.build()
    # print(president)
    # print(ld.G)
    # print(ld.ancestors)
    for aa, bb in zip(a, b):
        print(YES if ld.isOnPath(aa - 1, president, bb - 1) else NO)
    # print(YES if ld.isOnPath(14, 3, 7) else NO)
