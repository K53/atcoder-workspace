...
import sys
class FFT():
    def primitive_root_constexpr(self, m):  # 原始根を求める
        if m == 2: return 1
    ...
    def butterfly(self, a: "list[int]"): # フーリエ変換 元の列を直接書き換える
        n = len(a)
        h = (n-1).bit_length()
    ...
    def butterfly_inv(self, a: "list[int]"): # 元の列を直接書き換える
        n = len(a)
        h = (n - 1).bit_length()
